...
const SubCompetencyDetailPage: React.FC = () => {
  ...
  const { currentUser, evidence, evidenceVersion, addEvidenceItem, updateEvidenceItemRAG, deleteEvidenceItem } = useAuth(); 
  ...
  const handleDeleteEvidence = async (itemId: string) => {
    if (!competency) return;
    if (window.confirm('Are you sure you want to delete this evidence? This action cannot be undone.')) {
      await deleteEvidenceItem(itemId, competency.Sub_Competency_Code);
    }
  };
